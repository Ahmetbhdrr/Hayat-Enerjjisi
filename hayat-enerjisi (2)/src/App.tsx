...
function AppContent() {
  ...
  const navItems = [
    ...
    { id: 'store', icon: ShoppingBag, label: 'Mağaza' },
    { id: 'home', icon: Trees, label: 'Ana Sayfa', isMain: true },
    { id: 'frequency', icon: Music, label: 'Frekans' },
    { id: 'answers', icon: Cookie, label: 'Şans Kurabiyesi' },
  ];
        ...
        >
          {navItems.map((item) => (
            <button
              ...
              onClick={() => setActiveTab(item.id)}
              className={cn(
                "flex flex-col items-center gap-0.5 transition-all duration-300 px-1.5 py-1 rounded-lg",
                item.isMain && "scale-125 -translate-y-2 bg-purple-500 text-white rounded-full w-12 h-12 justify-center shadow-lg border-4 border-white dark:border-black",
                activeTab === item.id && !item.isMain
                  ? "bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400" 
                  : !item.isMain && (activeTheme.type === 'image' || (activeTheme.type === 'color' && activeTheme.id !== 'default' && !['default', 'lavender_theme', 'mint_theme', 'sunset_theme', 'ocean_theme', 'rose_theme', 'amber_theme', 'slate_theme'].includes(activeTheme.id)))
                    ? "text-white/60 hover:text-white"
                    : !item.isMain ? "text-gray-400 dark:text-gray-500" : ""
              )}
            >
              <item.icon size={item.isMain ? 24 : 16} strokeWidth={activeTab === item.id || item.isMain ? 2.5 : 2} />
              {!item.isMain && <span className="text-[7px] font-bold uppercase tracking-tighter">{item.label}</span>}
            </button>
          ))}
        </nav>
      </div>
